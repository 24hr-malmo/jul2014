...
placePicker.pickPlace = function(usedSlots, collisionImagePath, deltaFactor, next) {
    ...
    fs.readFile(collisionImagePath, function(err, buffer){
        ...
        for (var y = 0; y < height; y++) {
            for (var x = 0; x < width; x++) {
                ...
                process.stdout.write(visualize);
                //pixel[3] + "");
            }
        ...
        if (unusedSlots.length == 0) {
            ...
            slot.x *= deltaFactor;
            slot.y *= deltaFactor;
            return next({free: unusedSlots.length, slot: slot});
...
var collisionImagePath = path.join(__dirname, '../../masks/test4.png');
var tempList = [];
for (var i = 0; i < 4; i++) {
    tempList.push(1);
}
