...
placePicker.pickPlace = function(usedSlots, deltaFactor, show) {
    ...
    usedSlots.forEach(function(used) {
        var x = Math.round(used.x / deltaFactor);
        var y = Math.round(used.y / deltaFactor);
    ...
    fs.readFile(collisionImagePath, function(err, buffer){
        ...
        var width = Math.round(img.width / deltaFactor);
        var height = Math.round(img.height / deltaFactor);
        ...
        if (unusedSlots.length == 0) {
            ...
            return resolve({width: width, height: height, free: unusedSlots.length, slot: slot, width: width, height: height});
